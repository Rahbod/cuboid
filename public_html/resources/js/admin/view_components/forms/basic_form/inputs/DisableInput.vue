<template>
    <div class="form-group" :class="[index == null?'row':'form-group-float']">
        ...
        <div :class="input_container_class">
            <input class="form-control" disabled :class="[hasError() ?'border-danger':'']" :type="info.type" :style="input_style"
                   v-model="input_value" :name="getName(info.name)" :id="getId(info.name)" :placeholder="lable+'...'">
